...
export const teachMode: Mode = {
  id: "teach",
  toolNames: [
    "grade_math", "code_sandbox", "retrieve_from_documents",
...
```typescript
const toolDefinitions = [gradeMathTool, codeSandboxTool, retrieveFromDocumentsTool, ...COURSE_TOOLS, ...DOCUMENT_TOOLS, ...MEMORY_TOOLS, ...ASSIGNMENT_TUTOR_TOOLS, /* ... */];
...
This is the canonical Phase 16 mode-tool-scoping use case: the tightest possible tool set enforced server-side. Even if the agent's prompt were compromised, `retrieve_from_documents` and mastery tools simply aren't in the registry for exam sessions.
