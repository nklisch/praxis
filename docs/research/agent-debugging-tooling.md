...
The hard constraint is local-first ownership. `docs/SPEC.md` says local
deployments keep all data on-device and telemetry is off by default. A
debugging harness should therefore capture full-fidelity local evidence by
default, because the evidence does not leave the user's computer. Sanitization
is an export/sharing concern, not a local capture requirement. Hosted or
...
5. What should be rejected because it conflicts with local-first ownership,
   full-fidelity local capture, or package boundaries?
...
| Fidelity and retention | Must preserve enough local detail to debug visual/agent failures, with bounded local retention and sanitization only for explicit export or sharing. |
...
- The current logger keeps normal app logs quieter than the proposed evidence
  bundle. The debugging harness does not need to inherit that as a capture
  requirement: local bundles can capture full prompt/message/model/tool content
  when a debug run asks for it, while export/share paths can sanitize later.
- `packages/desktop/electron/main/logger.ts` wraps pino, applies API-key and
  credential handling paths, supports child bindings, ingests renderer records,
...
|---|---|---|---|---|---|
| Current Praxis logger + pino | Local structured logging | [Pino site](https://getpino.io/), [pino redaction docs](https://github.com/pinojs/pino/blob/main/docs/redaction.md), [pino child logger docs](https://github.com/pinojs/pino/blob/main/docs/child-loggers.md), [pino transport docs](https://github.com/pinojs/pino/blob/main/docs/transports.md). Accessed 2026-06-01. | Strong. Already installed and wrapped by Praxis; supports JSON records, child bindings, local file transports, and existing secret-handling behavior. | **Adopt** as the log substrate. | Build correlation fields and evidence-bundle pointers around it. Do not replace it with a hosted log backend. Evidence bundles can capture full-fidelity local details even if normal app logs stay quieter. |
...
| Sentry / Datadog-style observability | General error/performance observability | [Pino transport list](https://github.com/pinojs/pino/blob/main/docs/transports.md). Accessed 2026-06-01. | Low for the agent-debugging evidence standard. | **Reject for this feature.** | These are useful production operations tools, not the source of truth for prompt/tool/replay evidence. Any future export must be explicit opt-in and either sanitized for sharing or deliberately raw. |
...
- A local replay manifest that points to full-fidelity JSONL/event slices,
  DB snapshots or query summaries, browser traces when present, and a short
  failure summary.
- Capture and retention policy in the bundle schema. Local debug bundles should
  keep the prompt/message/model/tool/UI details needed to reproduce the failure;
  sanitization applies only when exporting or sharing a bundle off-machine.
...
| Failure replay | Build in-house local bundles. | Bundle manifests should point to full-fidelity local JSONL, event slices, DB state summaries, and optional browser traces. |
...
| Vendor exports | Optional only. | Exports must be explicit, sanitized for sharing, documented, and disabled by default. |
...
- Do not add Playwright or Browser Mode in the research feature. Add browser
  ...
  traces.
- Do store full-fidelity prompt/message/model/tool/UI evidence in local debug
  bundles when it helps reproduce the failure; the premise is that the bundle
  stays on the user's computer.
- Do not export Playwright traces casually. DOM snapshots and screenshots can
  contain private local data and must be sanitized or deliberately accepted
  before off-machine sharing.
