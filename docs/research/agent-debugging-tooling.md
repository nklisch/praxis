...
| Playwright | Browser automation and replay-oriented test traces | [Playwright Trace Viewer docs](https://playwright.dev/docs/trace-viewer-intro). Accessed 2026-06-01. | Strong for browser-driven student simulation and UI failure replay. Trace Viewer exposes action timeline, logs, source, network, errors, console, and interactive DOM snapshots. | **Add dependency if justified** by the student-simulation or UI replay feature. | Best fit when a downstream feature needs whole-app browser automation and `.trace.zip` artifacts. Trace files can include DOM/screenshot-like state, so they must stay local by default. |
...
- Add Playwright when the student-simulation or failure-replay features need
  whole-app browser automation and `.trace.zip` artifacts.
...
| Student simulation | Add Playwright if the implementation needs full browser control. | Keep traces local; retain only on failure or explicit capture by default. |
