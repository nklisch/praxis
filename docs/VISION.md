...
**Sessions are named arcs, not infinite chat.** A real tutor doesn't have one infinite conversation with a student; they have *lessons* — each with a before (today we'll work on X), a middle (the work), and an after (here's what we covered, here's what to practice). Praxis sessions are the same: each one has a topic, a structure, a beginning, and an end. They become artifacts you can revisit, not transcripts that scroll into oblivion. Multiple sessions of any mode can be open simultaneously as parallel tabs — algebra teach in one, calculus course-create in another — because real learners switch contexts.
