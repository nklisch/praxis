...
import { ConceptPicker, type ConceptPickerOption } from "../components/concept-picker.js";
...
describe("ConceptPicker", () => {
  it("renders the search input", () => {
    render(
      <ConceptPicker selectedIds={[]} options={OPTIONS} onChange={vi.fn()} placeholder="Search…" />,
    );
  ...
  it("selects an option on click", () => {
    ...
    render(<ConceptPicker selectedIds={[]} options={OPTIONS} onChange={onChange} />);
    fireEvent.focus(screen.getByRole("combobox"));
  ...
  it("renders chips for selected ids and supports removal", () => {
    const onChange = vi.fn();
    render(<ConceptPicker selectedIds={["var", "eq"]} options={OPTIONS} onChange={onChange} />);
  ...
  it("marks already-selected options as aria-disabled and ignores Enter on them", () => {
    const onChange = vi.fn();
    render(<ConceptPicker selectedIds={["var"]} options={OPTIONS} onChange={onChange} />);
    fireEvent.focus(screen.getByRole("combobox"));
  ...
  it("click-outside closes the dropdown", () => {
    render(
      <div>
        <ConceptPicker selectedIds={[]} options={OPTIONS} onChange={vi.fn()} />
