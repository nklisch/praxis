...
describe("NoteEditorSketch — convert button visibility", () => {
  it("shows the convert button when onConvertToConceptMap is provided", () => {
    render(
      <NoteEditorSketch noteId={NOTE_ID} onSave={noop} onConvertToConceptMap={vi.fn()} />,
    );
    expect(screen.getByRole("button", { name: /convert to concept map/i })).toBeDefined();
  });

  it("hides the convert button when onConvertToConceptMap is absent", () => {
    render(<NoteEditorSketch noteId={NOTE_ID} onSave={noop} />);
    expect(screen.queryByRole("button", { name: /convert to concept map/i })).toBeNull();
  });
});
...
describe("NoteEditorSketch — confirmation modal", () => {
  it("opens the modal when the convert button is clicked", async () => {
    ...
    const btn = screen.getByRole("button", { name: /convert to concept map/i });
    fireEvent.click(btn);
    await waitFor(() => {
  ...
  it("clicking Cancel closes the modal without calling onConvertToConceptMap", async () => {
    ...
    render(
      <NoteEditorSketch noteId={NOTE_ID} onSave={noop} onConvertToConceptMap={onConvert} />,
    );
    fireEvent.click(screen.getByRole("button", { name: /convert to concept map/i }));
