...
export function assembleBlocks(
  ...
  all.sort(
    (a, b) => FRAGMENT_ORDER.indexOf(a.positionLabel) - FRAGMENT_ORDER.indexOf(b.positionLabel),
  );
  return all;
}
 ...
 * for save.
 */
export function PromptBlockStack({ modeId, onModeChange }: PromptBlockStackProps): JSX.Element {
  ...
  const draftGlobal = editingBlock?.saveAction === "global" ? editingDraft : undefined;
  const draftAppend = editingBlock?.saveAction === "append" ? editingDraft : undefined;
