/**
 * Integration tests: envelope wiring for praxis.documents.* invoke channels.
 *
 * Channels exercised:
 *   praxis.documents.list   — no-payload getter; bare wrapEnvelope
 *   praxis.documents.get    — string payload; handleEnvelope + z.string().min(1)
 *   praxis.documents.delete — string payload; handleEnvelope + z.string().min(1)
 *
...
function makeServices(
  overrides: {
    ...
    documentsGet?: (id: unknown) => Promise<unknown>;
    documentsDelete?: (id: unknown) => Promise<unknown>;
  } = {},
) {
  const session = {
  ...
  const documents = {
    ...
    get: overrides.documentsGet
      ? vi.fn().mockImplementation(overrides.documentsGet)
      : vi.fn().mockResolvedValue(null),
    delete: overrides.documentsDelete
      ? vi.fn().mockImplementation(overrides.documentsDelete)
...
// ── praxis.documents.list — no-payload envelope ───────────────────────────────
...
describe("praxis.documents.list — envelope wiring", () => {
  ...
  it("resolves with { ok: true, value: <summaries> } when documents exist", async () => {
    ...
    const result = await handler?.({});
    expect(result).toMatchObject({ ok: true, value: summaries });
  });
...
// ── praxis.documents.get — string-payload envelope ────────────────────────────
...
describe("praxis.documents.get — envelope wiring", () => {
  it("resolves with { ok: true, value: <detail> } for a valid documentId", async () => {
    ...
    const handler = handlers.get("praxis.documents.get");
    expect(handler).toBeDefined();

    const result = await handler?.({}, "doc-1");
    expect(result).toMatchObject({ ok: true, value: detail });
    expect(services.documents.get).toHaveBeenCalledWith("doc-1");
  });
  ...
  it("resolves with { ok: true, value: null } when the document is not found", async () => {
    ...
    const result = await handler?.({}, "doc-missing");
  ...
  it("returns VALIDATION_FAILED for an empty string documentId", async () => {
    ...
    expect(services.documents.get).not.toHaveBeenCalled();
  });
...
// ── praxis.documents.delete — string-payload envelope ────────────────────────
...
describe("praxis.documents.delete — envelope wiring", () => {
  it("resolves with { ok: true, value: undefined } for a valid documentId", async () => {
    ...
    const handler = handlers.get("praxis.documents.delete");
    expect(handler).toBeDefined();

    const result = await handler?.({}, "doc-1");
    expect(result).toMatchObject({ ok: true });
    expect(services.documents.delete).toHaveBeenCalledWith("doc-1");
  });
  ...
  it("returns VALIDATION_FAILED for an empty string documentId", async () => {
    ...
    const result = await handler?.({}, "");
    expect(result).toMatchObject({ ok: false, error: { code: "VALIDATION_FAILED" } });
    expect(services.documents.delete).not.toHaveBeenCalled();
  });
