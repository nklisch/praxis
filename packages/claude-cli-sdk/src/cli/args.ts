...
import type { Options, ConversationOptions, OptionsBase, PermissionMode } from '../types/index.js';
import { isUUID } from '../types/options.js';
import { discoverTools, computeDisallowedTools } from '../discover.js';
...
async function buildCommonArgs(
  ...
  if (options.tools !== undefined) {
    if (options.tools === 'none') {
      ...
      if (only && deny) {
        throw new InvalidOptionError('tools', options.tools, 'only and deny are mutually exclusive');
      }
  ...
  if (options.jsonSchema) {
    if (options.maxTurns === 1) {
      throw new InvalidOptionError('jsonSchema', options.jsonSchema, 'requires maxTurns > 1 (Claude needs an agentic turn to produce structured output)');
    }
...
export async function buildCliArgs(prompt: string, options: Options = {}): Promise<BuiltArgs> {
  ...
  if (typeof options.tools === 'object' && 'custom' in options.tools && options.tools.custom?.length) {
 ...
 */
export async function buildConversationArgs(
