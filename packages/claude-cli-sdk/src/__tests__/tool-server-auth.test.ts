...
describe("startToolServer — auth gate", () => {
  ...
  it("auth frame then tool call → succeeds (happy path)", async () => {
    ...
    const result = await new Promise<{ success?: boolean; value?: unknown }>((resolve, reject) => {
    ...
    const result = await new Promise<{ success?: boolean; value?: unknown }>((resolve, reject) => {
      ...
      conn.on("connect", () => {
        ...
        setTimeout(() => {
          ...
          conn.write(`${JSON.stringify({ id: "split1", name: "echo", input: { s: "split" } })}\n`);
        }, 10);
      });
...
describe("startToolServer — socket permissions", () => {
  ...
  it.skipIf(os.platform() === "win32")("socket inode has 0600 permissions", async () => {
    handle = await startToolServer([echoTool]);
