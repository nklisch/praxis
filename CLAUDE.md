...
- **Native modules**: `better-sqlite3`, `canvas`, `isolated-vm` — see README "Native modules" for the dual-rebuild dance.
...
Praxis is built in numbered phases; each has a design doc in `docs/designs/`. Always check the relevant design before implementing. Phases 1–14 have shipped (foundation → engine layer → UI shell → verification tools → textbook RAG → course/lesson bootstrap → adaptive memory → multi-mode assessment → gates/progress map → knowledge-graph canonical pack → configure-mode authoring → workspace notes/flashcards → editorial foundation → tabs and library). Phases 1–15b have shipped (foundation → engine layer → UI shell → verification tools → textbook RAG → course/lesson bootstrap → adaptive memory → multi-mode assessment → gates/progress map → knowledge-graph canonical pack → configure-mode authoring → workspace notes/flashcards → editorial foundation → tabs and library → sketch/concept maps). Also shipped: **activity rail** (ambient progress surface — replaces the `IngestionProgress` modal; design in `docs/designs/activity-rail.md`). New design docs go in `docs/designs/phase-NN-*.md`; refactor plans in `docs/refactors/`.
