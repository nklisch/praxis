---
id: idea-share-getstudentid-helper-across-channels
kind: idea
tags: [refactor]
created: 2026-05-18
---
...
Total inline regressions across the 3 steps: ~42 (step 1: 1 in library,
step 2: 30 across memory/notes/flashcards/tabs/sketches/conceptMaps,
step 3: 11 across artifacts/author/session).

Cleanest fix: extract a shared helper module
...
Adopt across all channel files. Single commit, ~42 sites collapse to
single `getStudentId(services)` calls.

Story-sized. Mechanical. After this lands, ipc-server.ts itself can drop
its own local `getStudentId` closure (if any remained — verify) and use
the shared helper too.
